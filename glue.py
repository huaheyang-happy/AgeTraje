...
# Import the renamed class with an alias
from .data import AnnDatasetWithLabels as AnnDataset, ArrayDataset, DataLoader, GraphDataset, ParallelDataLoader
...
class GLUETrainer(Trainer):
    ...
    **kwargs
    ...
    def compute_losses(
        ...
        anneal = max(1 - (epoch - 1) / self.align_burnin, 0) if self.align_burnin is not None else 0
        if anneal > 0:
        ...
        if self.eidx is None or self.enorm is None or self.esgn is None:
            raise RuntimeError("Graph tensors (eidx, enorm, esgn) not set in trainer.")
        v = net.g2v(self.eidx, self.enorm, self.esgn)
        vsamp = v.rsample()

        g_nll = -net.v2g(vsamp, eidx, esgn).log_prob(ewt)
        pos_mask = (ewt > 0.5).to(torch.int64) # Assuming binary weights after sampling
        n_pos = pos_mask.sum().float()
        n_neg = (pos_mask == 0).sum().float()
        g_nll_pn = torch.zeros(2, dtype=g_nll.dtype, device=g_nll.device)
        g_nll_pn.scatter_add_(0, pos_mask, g_nll)
        # Normalize positive and negative NLL separately
        g_nll_pos_term = g_nll_pn[1] / n_pos if n_pos > 0 else torch.tensor(0.0, device=net.device)
        g_nll_neg_term = g_nll_pn[0] / n_neg if n_neg > 0 else torch.tensor(0.0, device=net.device)
        g_nll = g_nll_pos_term + g_nll_neg_term # Sum of averaged positive and negative NLL

        g_kl = D.kl_divergence(v, prior).sum(dim=1).mean() / vsamp.shape[0] # Normalize by num vertices
        ...
        x_kl = {
            k: D.kl_divergence(u[k], prior).sum(dim=1).mean() / x[k].shape[1] # Normalize by feature dim
            for k in net.keys
        }
    ...
    def format_data(self, data: List[torch.Tensor]) -> DataTensors:  # pragma: no cover
        ...
        keys = self.net.keys
        K = len(keys)
        # Assumes data loader yields [x_mod1, ..., x_modK, eidx, ewt, esgn]
        x, graph_data = data[:K], data[K:]
    ...
    def train_step(
        ...
        # Discriminator step
        losses_d = self.compute_losses(data, epoch, dsc_only=True)
        self.net.zero_grad(set_to_none=True)
        losses_d["dsc_loss"].backward()  # Already scaled by lam_align
        self.dsc_optim.step()

        # Generator/VAE step
        losses_g = self.compute_losses(data, epoch, dsc_only=False)
        self.net.zero_grad(set_to_none=True)
        losses_g["gen_loss"].backward()
        self.vae_optim.step()

        # Combine losses for logging
        losses_d.update(losses_g)
        return losses_d
    ...
    def val_step(
        ...
        self.net.eval()
        data = self.format_data(data)
        return self.compute_losses(data, engine.state.epoch, dsc_only=False)


    def fit(  # pylint: disable=arguments-renamed
        self,
        data: ArrayDataset, # Accepts ArrayDataset
        graph: GraphDataset,
        ...
        ----------
        data
            Data dataset (can be ArrayDataset or AnnDatasetWithLabels)
        graph
            Graph dataset
        val_split
            Validation split
        ...
        # Normalize graph edges and store tensors in the trainer
        ...
        # Split data and prepare shuffling
        ...
        # Create parallel data loaders
        train_loader = ParallelDataLoader(
            DataLoader(
                data_train,
                batch_size=config.DATALOADER_FETCHES_PER_BATCH,
                shuffle=True, # Let DataLoader handle inner shuffling if needed, Dataset handles outer
            ...
            cycle_flags=[False, True], # Cycle graph loader
        )
        val_loader = ParallelDataLoader(
            DataLoader(
                data_val,
                batch_size=config.DATALOADER_FETCHES_PER_BATCH,
                shuffle=True, # Shuffle validation set for better metric estimates?
                num_workers=config.DATALOADER_NUM_WORKERS,
                pin_memory=config.DATALOADER_PIN_MEMORY and not config.CPU_ONLY,
                drop_last=False,
                generator=torch.Generator().manual_seed(random_seed),
        ...
        # Call the base Trainer's fit method
        try:
            super().fit( # Call Trainer.fit, not GLUETrainer.fit
                train_loader,
    ...
    def get_losses(  # pylint: disable=arguments-differ
        ...
        # Prepare shuffling (needed for iterator creation)
        ...
        # Create parallel data loader for evaluation
        loader = ParallelDataLoader(
            DataLoader(
                data,
                batch_size=1, # Process one getitem_size chunk at a time
                shuffle=True, # Shuffle for representative estimate
                drop_last=False,
                pin_memory=config.DATALOADER_PIN_MEMORY and not config.CPU_ONLY,
                generator=torch.Generator().manual_seed(random_seed),
                persistent_workers=False,
        ...
        try:
            # Call the base Trainer's get_losses method
